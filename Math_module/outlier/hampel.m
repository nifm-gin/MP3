...
argI = 1;
while numel(varargin)>=argI
    if ~ischar(varargin{argI})
        error('PropertyName must be of type char.');
    end
    switch lower(varargin{argI})
        case 'standard' % nothing to be done
            argI = argI+1;
        case 'adaptive'
            if numel(varargin) < argI+1
                error('Optional input ''Adaptive'' must be followed by threshold value.');
            else
                % varargin{1}
                Option      = varargin{argI};
                % varargin{2}
                if isscalar(varargin{argI+1})
                    Threshold   = varargin{argI+1};
                else
                    error('Adaptive Threshold value must be a scalar.');
                end
            end
            argI = argI+2;
        case 'outliers'
            if numel(varargin) < argI+1
                error('Optional input ''Outliers'' must be followed by logical array or list of indices.');
            else
                % varargin{2}
                outliersTmp = varargin{argI+1}(:); % make this a column vector
                if islogical(outliersTmp)
                    if ~all(size(outliersTmp) == size(Y))
                        error('Logical outlier vector must have same size as Y.')
                    end
                    outliers = outliersTmp;
                elseif isnumeric(outliersTmp)
                    outliers(outliersTmp) = true;
                else
                    error('Outlier array must be either logical or numeric.')
                end
            end
            argI = argI+2;
    end
    ...
    switch lower(Option)
        case 'standard'
            for i = 1:numel(Y)
                ...
                [Y0(i), S0(i)]  = localwindow(X, Y, DX, i, include);
            end
        case 'adaptive'
            % Preallocate
            Y0Tmp   = S0;
            ...
            for i = 1:numel(Y)
                ...
                j       = 1;
                S0Rel   = inf;
                while S0Rel > Threshold && j <= numel(S0)
                    % Calculate Local Nominal Data Reference value
                    % and Local Scale of Natural Variation using DXTmp window
                    [Y0Tmp(j), S0Tmp(j)]    = localwindow(X, Y, DXTmp(j), i, include);
...
outl    = abs(Y - Y0) > T*S0;   % Index of possible outlier
YY(outl)= Y0(outl);              % Replace outliers with local median value
I(outl) = true;                 % Set Outlier detection
...
%% Built-in functions
    function [Y0, S0] = localwindow(X, Y, DX, i, include)
        % Index relevant to Local Window
        Idx = X(i) - DX <= X & X <= X(i) + DX & include;
        Y = sort(Y(Idx));
        N  = numel(Y);
...
%        S0  = 1.4826*median(abs(Y - Y0));
        
        % Use 0.25 and 0.75 quantiles to estimate std even more robustly
        q  = [0 100*(0.5:(N-0.5))./N 100]';
        Y  = [Y(1);Y(1:N);Y(N)];
        Y  = interp1q(q,Y,[25 75]');
        S0 =  1.4826*diff(Y)/2;   % robust estimation of the standard deviation
    end
