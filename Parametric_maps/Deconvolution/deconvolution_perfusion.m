function [CBV,CBF,MTT,TMAX,TTP,T0,CBVm,CBFm,MTTm,CBV_mask,CBF_mask,MTT_mask,volume_mask] = deconvolution_perfusion(AIF,VOX,TR,TE)
% function [CBV,CBF,MTT,TMAX,TTP,T0,CBVm,CBFm,MTTm,CBV_mask,CBF_mask,MTT_mask] = deconvolution_perfusion(AIF,VOX,dt,varargin)
...
M(mean(VOX(:,:,:,2:6),4) < max(max(max(mean(VOX(:,:,:,2:6),4))/(noise_lvl*2)))) = false;
M(mean(VOX(:,end-10:end),2) < max(max(max(mean(VOX(:,end-10:end),2))/(noise_lvl*2)))) = false;

%%% Voxels qui 'bougent peu'
M(min(VOX,[],4) > (mean(VOX(:,:,:,2:6),4)-2*std(VOX(:,:,:,2:6),0,4))) = false;
...
noise_level = median((mean(VOX(:,2:6),2) - min(VOX,[],2))./std(VOX(:,2:6),0,2));
