function [B, D, STATUS]=levenbergmarquardtASL(funct,xdat,ydat,B_init)
...
if isstruct(funct),
    ...
    pars       = struct(...
        ...
        'parsLim', [1000*max(ydat); 1; 5;10], ... % param limit
...
for i=1:pars.b_num
   ...
   % figure(102),plot(0:0.05:4-0.05,pars.F(xdat,pars.B)), hold on,plot(0:0.05:4-0.05,ydat,'+'), hold off
...
elseif pars.T2 < pars.T1               % some parameters have not yet converged and we still have iterations left.
    B          = pars.B + p;           % try new parameters by adding their partial derivatives
    if sum(abs(B) > pars.parsLim) ~=0
        pars.T2=pars.T1;
        pars.B=[0 0 0 0];
        pars.D=[0 0 0 0];
        return
