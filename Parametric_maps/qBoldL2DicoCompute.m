function DataOut = qBoldL2DicoCompute(S,ADC,Dico)
...
%     R2 = 1 - sum(bsxfun(@minus,Dico.dat(Idx(:,ADCix(SInd(a))),:),S(SInd(a),:)).^2,2)/sum((S(SInd(a),:) - mean(S(SInd(a),:))).^2);
    ...
    DataOut1(SInd(a)) = Dico.par(Jdx,1); 
%     DataOut1(SInd(a)) = Jdx; % Vf  % --> save the indice in the place of the BVf Jdx;
    DataOut2(SInd(a)) =Dico.par(Jdx,2); % Radius
    DataOut3(SInd(a)) = Dico.par(Jdx,3);% Dk
    DataOut4(SInd(a)) = R2m;
    
%     DataOut(SInd(a),1)    = Dico.par(Jdx,1); % Vf
%     DataOut(SInd(a),2)    = Dico.par(Jdx,2); % Radius
%     DataOut(SInd(a),3)    = Dico.par(Jdx,3);% Dk
%     DataOut(SInd(a),4)    = R2m;
%     end
%     figure;plot(Dico.dat(Jdx,:)); hold, plot(S); hold off
end
DataOut(:,1) = DataOut1;
DataOut(:,2) = DataOut2;
DataOut(:,3) = DataOut3;
