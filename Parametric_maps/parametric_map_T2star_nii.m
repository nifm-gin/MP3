function T2star_map = parametric_map_T2star_nii(MGE_map_filename, json_filename, Mask_filename, add_parameters)
...
fid=fopen(char(MGE_map_filename) ,'r');
if fid>0
    fclose(fid);
    data.hdr = spm_vol(char(MGE_map_filename));
...
% for later
mask_to_use(1:data.hdr(1).dim(1),1:data.hdr(1).dim(2))=1;

% initializing variables
% T2star_map.reco='';
T2star_map.data = NaN([data.hdr(1).dim 2]);     % parameters(T2, M0)
T2star_map.err = NaN([data.hdr(1).dim 2]);     % uncertainties on the parameters
T2star_map.wrongpix = NaN([data.hdr(1).dim]);  % map of excluded pixels
T2star_map.ki2 = NaN([data.hdr(1).dim]);       % Adjustment quality
echotime = data.json.EchoTime;                                                % echo time in ms

% if MGESE or MGEFIDSE
switch char(data.json.MethodDiffusion)  
    case 'eB_MGESE2' %{'MGEFIDSE', 'MGESE'}
