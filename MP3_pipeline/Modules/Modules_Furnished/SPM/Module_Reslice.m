...
if isempty(opt)
    ...
    user_parameter(:,3)   = {'Image to reslice','XScanOrXROI','','',{'SequenceName'},'Mandatory',...
...
if isempty(files_out)
    ...
    for i=1:length(files_in.In2)
        %i+1 becauce the first row corresponds to the reference image
        table_out_tmp = opt.Table_in(i+1,:);
        table_out_tmp.Path = categorical(cellstr([opt.folder_out, filesep]));
        table_out_tmp.SequenceName =  categorical(cellstr([opt.output_filename_ext, char(table_out_tmp.SequenceName)]));
...
for i=1:length(files_in.In2)
    ...
    source_filename = fullfile(char(opt.Table_out.Path(i)),[char(opt.Table_in.Patient(i+1)), '_', char(opt.Table_in.Tp(i+1)),'_', char(opt.Table_in.SequenceName(i+1)),'.nii']);
    if ~strcmp(files_in.In2{i}, source_filename)
        copyfile(files_in.In2{i},  source_filename)
        if exist(strrep(files_in.In2{i},'.nii','.json'), 'file')
            copyfile(strrep(files_in.In2{i},'.nii','.json'), strrep(source_filename, '.nii', '.json'))
        end
    end
    ...
    %% Json Processing
    if exist(strrep(files_in.In2{i},'.nii','.json'), 'file')
        [path, name, ~] = fileparts(files_in.In2{i});
    ...
    data_resliced = niftiread(fullfile(path, [opt.output_filename_ext, name,ext]));
    data_resliced_info = niftiinfo(fullfile(path, [opt.output_filename_ext, name,ext]));
    %% reshap data if needed (if dim >4)
    if length(data_to_reslice_info.ImageSize) > 4
        data_resliced = reshape(data_resliced, [data_resliced_info.ImageSize(1:3) data_to_reslice_info.ImageSize(4:end)]);
        data_resliced_info.ImageSize = [data_resliced_info.ImageSize(1:3) data_to_reslice_info.ImageSize(4:end)];
        tmp = zeros([1 length(data_to_reslice_info.ImageSize)]);
        tmp(1:length(data_resliced_info.PixelDimensions))=data_resliced_info.PixelDimensions ;
        data_resliced_info.PixelDimensions = tmp;
        
        % overwrite the tmp file by the smoothed one
        niftiwrite(data_resliced,fullfile(path, [opt.output_filename_ext, name,ext]), data_resliced_info)
    end
    
    
    movefile(fullfile(path, [opt.output_filename_ext, name,ext]), files_out.In1{i});
