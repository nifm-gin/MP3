function [files_in,files_out,opt] = Module_Export_XROI_Plot(files_in,files_out,opt)
...
%   
    ...
    user_parameter(:,3)   = {'   .ROI','XROI','','',{'SequenceName'},'Mandatory',...
...
UROI = unique(opt.Table_in.SequenceName(opt.Table_in.Type == 'ROI', :));

Mean_TPs = nan(length(UROI),length(UTp));
STD_TPs = nan(length(UROI),length(UTp));
TP = nan(length(UROI),length(UTp));
for k=1:length(UROI)
    for i=1:length(UTp)
        t = char(UTp(i));
        ...
        for j=1:length(UPat)
            ...
            ROIsDB = DBPatTp(DBPatTp.Type == 'ROI',:);
            ROIDB = ROIsDB(ROIsDB.SequenceName == UROI(k),:);
            if isempty(ROIDB)
                continue
        ...
        Mean_TPs(k,i) = nanmean(All_Vox_TP);
        STD_TPs(k,i) = nanstd(All_Vox_TP);
    end
end
...
figure
for k=1:length(UROI)
    errorbar(1:length(UTp),Mean_TPs(k,:), [], STD_TPs(k,:), '-*');
    hold on
end
legend(cellstr(UROI))
xticks(1:length(UTp))
