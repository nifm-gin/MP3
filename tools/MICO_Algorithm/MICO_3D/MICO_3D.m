...
function [M, b, C]=  MICO_3D(Img,W,M,C,b,Iter, iterCM,q, Fold)
for n = 1:Iter
    for kk=1:iterCM
        C = updateC(Img, W, q, b, M);   % Note: the order of updating C,M,and B can be changed.
        M = updateM(Img, W, C,b,q);
    end    
    b = updateB(Img, C, M,q, Fold);
end


%%%%%%%%%%%%%%%%%%
%%%  update b  %%%
function b =updateB(Img, C, M,q, Fold)
...
D=zeros(N_bas,1);
for i=1:N_bas
    filename = [Fold, 'basis_',num2str(i),'.mat'];
    load(filename);
    basis_i = basis;
    A=Img.*basis_i.*PC;
    D(i)=sum(A(:));  
    for j=i:N_bas
        filename = [Fold, 'basis_',num2str(j),'.mat'];
        load(filename);
        basis_j = basis;
...
b=zeros(size(Img));
for kk=1:N_bas
    filename = [Fold, 'basis_',num2str(kk),'.mat'];
    load(filename);
    b=b+coeff(kk)*basis;
end
