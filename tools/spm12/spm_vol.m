...
elseif iscell(P)
    ...
    cnt = 0;
    for i=1:size(P,1)
        if ~exist(P(i,:),'file') && exist(strrep(P(i,:), '.nii', '.nii.gz'),'file')
            gunzip(strrep(P(i,:), '.nii', '.nii.gz'))
        end
...
if isempty(n) && length(V.private.dat.dim) > 3
    ...
    if length(V.private.dat.dim)>4
        ind = 1;
        for j=1:V.private.dat.dim(5)
            for i=1:V.private.dat.dim(4)
                V0(ind) = spm_vol_nifti(V.private, [i,j]);
                ind = ind+1;
            end
        end
    else
