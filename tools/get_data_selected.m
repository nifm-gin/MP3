...
if get(handles.MP3_scans_button, 'Value') == 0  % search inside the scan listing
    is_scan =  handles.database.Type == 'Scan';
    nii_listing = handles.database.SequenceName(Patient_filter & tp_filter & is_scan);
    
elseif get(handles.MP3_VOIs_button, 'Value') == 0  % search inside the ROI listing
...
elseif get(handles.MP3_Others_button, 'Value') == 0    % search inside "Other" tag listing
     is_other = handles.database.Type ~= 'Scan' & handles.database.Type ~= 'ROI';
     nii_listing = handles.database.SequenceName(Patient_filter & tp_filter & is_other);
end
