function [recdat, par] = GFB_convert_read_Uvascim2(filename, par, applyRescale)
...
% In therory matlab store data in 32 bits
if isa(fixed.uvascim.image.reco.data,'single');
    par.scn.pix_bits = 32;
end
...
imgTypes = [allInfo.image_type_mr]';
[~,~,imgTlookup] = unique(imgTypes); % create lookup index
imgTypes = unique(imgTypes(imgIsAcquired)); % list only different image types
...
%thisDValIdx
%     thisData(:) = squeeze(uvascim.image.reco.data(:,:,thisEchoIdx,thisSliceNum,thisDValIdx));
    thisData(:) = squeeze(fixed.uvascim.image.reco.data(:,:,thisEchoIdx,thisSliceNum,thisScanIdx));
    thisData = rot90(thisData,1);
%     thisData = thisData';
