                ...
                for k=1:length(listScans)
                    ...
                    if length(NbValeurs)<6
                        for l=1:length(NbValeurs)
                            ...
                            Tags = table();
                            if isempty(listTP(j).name)
                                listTP(j).name = 'ses-J1';
                            end
                            ...
                            PrefixScanName = [listPatients(i).name, '_', listTP(j).name, '_'];
                            nameSeq = listScans(k).name((length(listPatients(i).name) + length(listTP(j).name) + 3):end-4);
                            if ~startsWith(listScans(k).name, PrefixScanName)
                                nameSeq = listScans(k).name((length(listPatients(i).name) + 2):end-4);
                                PrefixScanName = [listPatients(i).name, '_'];
                                if ~startsWith(listScans(k).name, PrefixScanName)
                                    nameSeq = listScans(k).name(1:end-4);
                                end
                            end
%                             nameSeq = listScans(k).name((length(listPatients(i).name) + length(listTP(j).name) + 3):end-4);
%                             if isempty(nameSeq)
...
%                                 end
%                             end
                            Tags.Filename = categorical(cellstr([listPatients(i).name(5:end), '_', listTP(j).name(5:end), '_', nameSeq, '_', num2str(NbValeurs(l))]));
                            Tags.Type = categorical(cellstr('ROI'));
                            Tags.IsRaw = categorical(1);
                            Tags.SequenceName = categorical(cellstr([nameSeq, '_', num2str(NbValeurs(l))]));
                            filename = [PathProject, 'ROI_data', filesep, listPatients(i).name(5:end), '_', listTP(j).name(5:end), '_', nameSeq, '_', num2str(NbValeurs(l)), '.nii'];
...
%                             end
%                         end
                        Tags.Filename = categorical(cellstr([listPatients(i).name(5:end), '_', listTP(j).name(5:end), '_', nameSeq]));
                        Tags.Type = categorical(cellstr('Scan'));
                        Tags.IsRaw = categorical(1);
                        Tags.SequenceName = categorical(cellstr(nameSeq));
