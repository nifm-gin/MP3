...
% Last Modified by GUIDE v2.5 23-Oct-2017 11:00:34
...
for x = 1:numel(filename)
    ...
    if fid>0
        ...
        if ~isfield(roi_loaded, 'uvascroi')
            warndlg(strcat('Somthing wrong with the ROI file:', {' '},filename{x}),'Warning');
            return
        end
    else
        warndlg(strcat('Somthing wrong with the ROI file:', {' '},filename{x}),'Warning');
...
% --------------------------------------------------------------------
function MIA_add_scan_Callback(hObject, eventdata, handles)
% hObject    handle to MIA_add_scan (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isfield(handles, 'database')
    return
...
if get(handles.MIA_scan_VOIs_button, 'Value') == 0  % add news Scan
    %% speed to dir function to replace  mat_list=dir('*.mat')
    cellf = @(fun, arr) cellfun(fun, num2cell(arr), 'uniformoutput',0);
    mat_list=cellf(@(f) char(f.tochar()), java.io.File(pwd).list());
    mat_list = cell2struct(mat_list, 'name' , size(mat_list,1));
    %%
    list_scan(1:numel(mat_list)) = {mat_list.name};
    list_scan = sort(list_scan(3:end));
%     list_scan(numel(list_scan)+1) = {'Other'};
    if ~exist('list_scan', 'var')
        for i = 1:numel(mat_list)
            list_scan(scan_number) = {mat_list(i).name};
            scan_number = scan_number+1;
        end
    end
    scan_list  = [sort(handles.clips(:,1))' 'Other'];
    [scan, ok1] = listdlg('PromptString','Select a parameter:',...
        'Name', 'Select a Parameter',...
        'SelectionMode','single',...
        'ListSize', [400 300],...
        'ListString',scan_list);
    if ok1 == 0
        return
    end
%     
%     idx = ~cellfun(@isempty,strfind(list_scan',scan_list{scan}));
%     list_scan =list_scan(idx);
%     list_scan(numel(list_scan)+1) = {'Other'};
%     
    if strcmp('Other', scan_list(scan)) ==1
        newparameter = inputdlg('Name of the new parameter ', 'Question?', 1, {''});
        new =1;
    else
        newparameter = scan_list(scan);
        new =0;
    end
    [file, ok2] = listdlg('PromptString', strcat({'Select a file for: '}, newparameter),...
        'SelectionMode','single',...
        'ListSize', [400 300],...
        'ListString',list_scan(:));
    if ok2 == 0
        return
    end
    
%     %%%%%%%% if you want to display evey .mat option "Other"
%     if strcmp(list_scan(file), 'Other')
%         for i = 1:numel(mat_list)
%             list_scan(scan_number) = {mat_list(i).name};
%             scan_number = scan_number+1;
%         end
...
%     end
    ...
    if ok1 == 1 && ok2 ==1
        if ~isfield(handles.database(patient).day(time_point), 'parameters')
            handles.database(patient).day(time_point).parameters = newparameter;
            handles.database(patient).day(time_point).scans_file = list_scan(file);
        else
            handles.database(patient).day(time_point).parameters = [handles.database(patient).day(time_point).parameters newparameter];
            handles.database(patient).day(time_point).scans_file = [handles.database(patient).day(time_point).scans_file list_scan(file)];
        end
    end
    if new == 1
        handles.clips(size(handles.clips,1)+1,1) = newparameter;
        handles.clips(size(handles.clips,1),2) = {0};
        handles.clips(size(handles.clips,1),3) = {100000};
    end
else % add news VOIs
    
    mask_list=dir('*.mat');
    list_scan(1:numel(mask_list)) = {mask_list.name};
    list_scan(numel(mask_list)+1) = {'Other'};
    if ~exist('list_scan', 'var')
        for i = 1:numel(mask_list)
            list_scan(scan_number) = {mask_list(i).name};
            scan_number = scan_number+1;
        end
    end
    [scan, ok1] = listdlg('PromptString','Select a VOI:',...
        'SelectionMode','single',...
        'ListSize', [180 200],...
    ...
    if strcmp('Other', handles.VOIs(scan)) ==1
        newVOI = inputdlg('Name of the new VOI name ', 'Question?', 1, {''});
        handles.VOIs = [handles.VOIs(1:end-1), newVOI, handles.VOIs(end-1:end)];
        scan = numel(handles.VOIs)-2;
        
    end
    
    
    idx = ~cellfun(@isempty,strfind(list_scan',handles.VOIs{scan}));
    list_scan =list_scan(idx);
    list_scan(numel(list_scan)+1) = {'Other'};
    if scan < numel(handles.VOIs)
        [file, ok2] = listdlg('PromptString','Select a file:',...
            'SelectionMode','single',...
            'ListSize', [400 300],...
            'ListString',list_scan(:));
        if ok2 == 0
        ...
        if ok1 == 1 && ok2 ==1
            for i = 1:numel(time_point)
                if ~isfield(handles.database(patient).day(time_point(i)), 'VOIs')
                    handles.database(patient).day(time_point(i)).VOIs = handles.VOIs(scan);
                    handles.database(patient).day(time_point(i)).VOIs_file = list_scan(file);
                else
                    handles.database(patient).day(time_point(i)).VOIs = [handles.database(patient).day(time_point(i)).VOIs handles.VOIs(scan)];
                    handles.database(patient).day(time_point(i)).VOIs_file = [handles.database(patient).day(time_point(i)).VOIs_file list_scan(file)];
                end
            end
            set(handles.MIA_scans_list, 'Value', numel(handles.database(patient).day(time_point(i)).VOIs));
        end
    else   % other VOIs 'home made'
        %%% a faire!!!!
    end
end

guidata(hObject, handles);
if numel(time_point) > 1
    set(handles.MIA_time_points_list, 'Value', time_point(1));
end
MIA_update_database_display(hObject, eventdata, handles)
cd(path_root);
...
% --------------------------------------------------------------------
function MIA_rename_scan_Callback(hObject, eventdata, ~)
...
timepoint = get(handles.MIA_time_points_list, 'Value');
scan_name = get(handles.MIA_scans_list, 'Value');
...
if get(handles.MIA_scan_VOIs_button, 'Value')
    ...
    name_option = [sort(handles.clips(:,1))' 'Other'];
    old_scan_name = handles.database(patient).day(timepoint).parameters(scan_name);
end
...
handles = guidata(hObject);
% patient = get(handles.MIA_name_list, 'Value');
new_patient_directory = uigetdir(pwd, 'Select Directory');
...
if handles.mode == 1
    ...
    if ~isfield(handles, 'data_selected_for_PRM_resized')
        %         warndlg(strcat('There are more than 1 scan type per time point'), 'Warning');
        return
    end
    list_day = ['-1', {handles.data_selected_for_PRM_resized.image.day}];
    set(handles.MIA_PRM_ref_popupmenu, 'String', list_day', 'Value', 2);
    %set MIA_PRM_slider
    set(handles.MIA_PRM_slider_tp, 'Max', numel(handles.data_selected_for_PRM.image));
    set(handles.MIA_PRM_slider_tp,'Value',1);
    set(handles.MIA_PRM_slider_tp,'Min',1);
    if numel(handles.data_selected_for_PRM.image) == 2
        set(handles.MIA_PRM_slider_tp,'Visible', 'off');
    else
        set(handles.MIA_PRM_slider_tp,'Visible', 'on');
        set(handles.MIA_PRM_slider_tp,'SliderStep',[1/(numel(handles.data_selected_for_PRM.image)-1) min(5/(numel(handles.data_selected_for_PRM.image)-1),1)]);
    end
    
    MIA_update_axes_PRM(hObject, eventdata, handles)
end
...
for i = 1:numel(data_selected)
    ...
    if fid_nii>0 
        ...
        warndlg('Somthing wrong with the data. Nii of json file is missing','Warning');
        return
    end
    guidata(hObject, handles);  
end
...
%     if handles.data_loaded.number_of_scan > i-1 && numel(handles.data_loaded(i).V.dim) > 5
% %         if isfield(handles.data_selected.image(i).reco, 'color_map') && strcmp(handles.data_selected.image(i).reco.color_map, 'Color')
% %             set(eval(['handles.MIA_data', stri, '_expt_slider']), 'Visible', 'off');
% %             set(eval(['handles.MIA_data', stri, '_expt_slider']), 'Value', 1);
% %         else
%             set(eval(['handles.MIA_data', stri, '_expt_slider']), 'Visible', 'on', 'Value', 1, 'Min', 1, 'Max', size(handles.data_loaded(i).nii, 5),...
%                 'SliderStep',[1/(size(handles.data_loaded(i).nii, 5)-1) min(5/(size(handles.data_loaded(i).nii, 5)-1),1)]);
% %         end
%     else
%         set(eval(['handles.MIA_data', stri, '_expt_slider']), 'Visible', 'off');
%         set(eval(['handles.MIA_data', stri, '_expt_slider']), 'Value', 1);
...
% --- Executes on selection change in MIA_resolution_popupmenu.
function MIA_resolution_popupmenu_Callback(hObject, eventdata, handles, data_in_table)
% hObject    handle to MIA_resolution_popupmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns MIA_resolution_popupmenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from MIA_resolution_popupmenu
if ~isfield(handles, 'data_loaded') && ~isfield(handles, 'data_selected_for_PRM')
    return
end
% resize data
res_option = get(handles.MIA_resolution_popupmenu, 'Value');
handles.resolution_selected = handles.resolution(res_option);
% resized and Align slices for each mode (single or PRM)
switch handles.mode
    case 1 % case single mode
        % "Original" resolution selected and 1 scan selected
        if  handles.resolution_selected == 1 && handles.data_loaded(1).number_of_scan== 1
            handles.resolution_selected = handles.data_selected.scan_info(:,1);
            % "Original" resolution selected and more than 1 scan selected --> use the highest resolution
        elseif  handles.resolution_selected == 1 && handles.data_loaded(1).number_of_scan >1
            handles.resolution_selected = max(handles.data_selected.scan_info(:,1));
        else
            handles.resolution_selected = handles.resolution(res_option);
        end
        handles.data_selected_resized.scan_info(:,1:2) = handles.resolution_selected;
        
        for ii=1:handles.data_loaded(1).number_of_scan
            imgsize = handles.data_selected.scan_info(ii,:);
            factor = [handles.resolution_selected/handles.data_selected.scan_info(ii,1) handles.resolution_selected/handles.data_selected.scan_info(ii,2)];
            data = zeros([ceil(factor(1)*imgsize(1)),ceil(factor(2)*imgsize(2)),imgsize(3:5)]);
            for a=1:size(handles.data_selected.image(ii).reco.data,3)
                for b=1:size(handles.data_selected.image(ii).reco.data,4) % slice
                    for c=1:size(handles.data_selected.image(ii).reco.data,5)
                        data(:,:,a,b,c) = imresize(handles.data_selected.image(ii).reco.data(:,:,a,b,c),[size(data, 1) size(data,2)],'bilinear'); %bilinear
                    end
                end
            end
            handles.data_selected_resized.image(ii).reco.data =  data;
        end
        % align
        if handles.data_loaded(1).number_of_scan > 1
            handles.data_selected_resized.z_offset = handles.data_selected.z_offset;
            thickness_sorted = sort(handles.data_selected.scan_info(:,8));
            slice_to_remove = [];
            for i=1:numel(unique(thickness_sorted))
                match_scan = abs(handles.data_selected.scan_info(:,8) - thickness_sorted(i)) < 0.00001;
                match_scan = find(match_scan== 1);
                for j = 1:numel(match_scan)
                    scan_nbr = match_scan(j);
                    matrix_size = size(handles.data_selected_resized.image(scan_nbr).reco.data);
                    if size(matrix_size,2) == 2
                        matrix_size(3) = 1;
                    end
                    matrix_size(4) = numel(handles.data_selected_resized.z_offset);
                    tmp=NaN(matrix_size);
                    % first data (smaller slice thickness)
                    if i == 1 && j == 1
                        for jj = 1:numel(handles.data_selected.scan_z_offset(scan_nbr).values)
                            match =  abs(handles.data_selected.z_offset - handles.data_selected.scan_z_offset(scan_nbr).values(jj)) <  1e-3;
                            match_1 = find(match== 1);
                            tmp(:,:,:,match_1,:,:) = handles.data_selected_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:); %#ok<FNDSB>
                        end
                    else
                        old_match = 0;
                        for jj = 1:size(handles.data_selected.image(scan_nbr).reco.data, 4)
                            match =  abs(handles.data_selected.z_offset - handles.data_selected.scan_z_offset(scan_nbr).values(jj)) <  1e-3;
                            match_1 = find(match== 1);
                            if match_1 ~= old_match+1
                                for x = (old_match+1):match_1-1
                                    if (handles.data_selected.scan_z_offset(scan_nbr).values(jj)-handles.data_selected.scan_info(scan_nbr,8)/2 < handles.data_selected.z_offset(x) - min(handles.data_selected.scan_info(:,8))/2) ||...
                                            abs((handles.data_selected.scan_z_offset(scan_nbr).values(jj)-handles.data_selected.scan_info(scan_nbr,8)/2)- (handles.data_selected.z_offset(x) - min(handles.data_selected.scan_info(:,8))/2)) <  1e-3
                                        tmp(:,:,:,x,:,:) = handles.data_selected_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:);
                                        slice_to_remove = [slice_to_remove match_1];
                                    end
                                end
                                tmp(:,:,:,match_1,:,:) = handles.data_selected_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:);
                            else
                                tmp(:,:,:,match_1,:,:) = handles.data_selected_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:);
                            end
                            for xx = match_1:numel(handles.data_selected.z_offset)
                                if (handles.data_selected.scan_z_offset(scan_nbr).values(jj)+handles.data_selected.scan_info(scan_nbr,8)/2 > handles.data_selected.z_offset(xx)+  min(handles.data_selected.scan_info(:,8))/2) ||...
                                        abs((handles.data_selected.scan_z_offset(scan_nbr).values(jj)+handles.data_selected.scan_info(scan_nbr,8)/2) - (handles.data_selected.z_offset(xx)+  min(handles.data_selected.scan_info(:,8))/2)) <  1e-3
                                    tmp(:,:,:,xx,:,:) = handles.data_selected_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:);
                                end
                            end
                            old_match = match_1;
                        end
                    end
                    handles.data_selected_resized.image(scan_nbr).reco.data = tmp;
                end
                
            end
            if ~isempty(slice_to_remove)
                slice_to_remove = unique(slice_to_remove);
                for i = 1 : numel(handles.data_selected_resized.image)
                    handles.data_selected_resized.image(i).reco.data(:,:,:,slice_to_remove,:,:) = [];
                end
                handles.data_selected_resized.z_offset(slice_to_remove) = [];
                handles.data_selected_resized.scan_info(:,4) = handles.data_selected_resized.scan_info(1,4) - numel(slice_to_remove);
            end
        end
    case 2 % case PRM mode
        % resized
        % "Original" resolution selected
        if  handles.resolution_selected == 1
            handles.resolution_selected = max(handles.data_selected_for_PRM.scan_info(:,1));
        ...
        handles.data_selected_for_PRM_resized.scan_info(:,1:2) = handles.resolution_selected;
        
        for ii=1:handles.data_selected_for_PRM.scan_number
            imgsize = handles.data_selected_for_PRM.scan_info(ii,:);
            
            factor = [handles.resolution_selected/handles.data_selected_for_PRM.scan_info(ii,1) handles.resolution_selected/handles.data_selected_for_PRM.scan_info(ii,2)];
            data = zeros([ceil(factor(1)*imgsize(1)),ceil(factor(2)*imgsize(2)),imgsize(3:5)]);
            for a=1:size(handles.data_selected_for_PRM.image(ii).reco.data,3)
                for b=1:size(handles.data_selected_for_PRM.image(ii).reco.data,4) % slice
                    for c=1:size(handles.data_selected_for_PRM.image(ii).reco.data,5)
                        data(:,:,a,b,c) = imresize(handles.data_selected_for_PRM.image(ii).reco.data(:,:,a,b,c),[size(data, 1) size(data,2)],'bilinear'); %bilinear
                    end
                end
            end
            handles.data_selected_for_PRM_resized.image(ii).reco.data =  data;
        ...
        % align
        if handles.data_selected_for_PRM.scan_number > 1
            handles.data_selected_for_PRM_resized.z_offset = handles.data_selected_for_PRM.z_offset;
            thickness_sorted = sort(handles.data_selected_for_PRM.scan_info(:,8));
            slice_to_remove = [];
            for i=1:numel(unique(thickness_sorted))
                match_scan = abs(handles.data_selected_for_PRM.scan_info(:,8) - thickness_sorted(i)) < 0.00001;
                match_scan = find(match_scan== 1);
                for j = 1:numel(match_scan)
                    scan_nbr = match_scan(j);
                    matrix_size = size(handles.data_selected_for_PRM_resized.image(scan_nbr).reco.data);
                    matrix_size(4) = numel(handles.data_selected_for_PRM.z_offset);
                    tmp=NaN(matrix_size);
                    % first data (smaller slice thickness)
                    if i == 1 && j == 1
                        for jj = 1:numel(handles.data_selected_for_PRM.scan_z_offset(scan_nbr).values)
                            match =  abs(handles.data_selected_for_PRM.z_offset - handles.data_selected_for_PRM.scan_z_offset(scan_nbr).values(jj)) <  1e-3;
                            match_1 = find(match== 1);
                            tmp(:,:,:,match_1,:,:) = handles.data_selected_for_PRM_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:); %#ok<FNDSB>
                        end
                    else
                        ...
                        for jj = 1:size(handles.data_selected_for_PRM.image(scan_nbr).reco.data, 4)
                            ...
                            if match_1 ~= old_match+1
                                for x = (old_match+1):match_1-1
                                    if (handles.data_selected_for_PRM.scan_z_offset(scan_nbr).values(jj)-handles.data_selected_for_PRM.scan_info(scan_nbr,8)/2 < handles.data_selected_for_PRM.z_offset(x) - min(handles.data_selected_for_PRM.scan_info(:,8))/2) ||...
                                            abs((handles.data_selected_for_PRM.scan_z_offset(scan_nbr).values(jj)-handles.data_selected_for_PRM.scan_info(scan_nbr,8)/2)- (handles.data_selected_for_PRM.z_offset(x) - min(handles.data_selected_for_PRM.scan_info(:,8))/2)) <  1e-3
                                        tmp(:,:,:,x,:,:) = handles.data_selected_for_PRM_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:);
                                        slice_to_remove = [slice_to_remove match_1];
                                    end
                                end
                            ...
                            for xx = match_1:numel(handles.data_selected_for_PRM.z_offset)
                                if (handles.data_selected_for_PRM.scan_z_offset(scan_nbr).values(jj)+handles.data_selected_for_PRM.scan_info(scan_nbr,8)/2 > handles.data_selected_for_PRM.z_offset(xx)+  min(handles.data_selected_for_PRM.scan_info(:,8))/2) ||...
                                        abs((handles.data_selected_for_PRM.scan_z_offset(scan_nbr).values(jj)+handles.data_selected_for_PRM.scan_info(scan_nbr,8)/2) - (handles.data_selected_for_PRM.z_offset(xx)+  min(handles.data_selected_for_PRM.scan_info(:,8))/2)) <  1e-3
                                    tmp(:,:,:,xx,:,:) = handles.data_selected_for_PRM_resized.image(scan_nbr).reco.data(:,:,:,jj,:,:);
                                end
                            end
                            old_match = match_1;
            ...
            if ~isempty(slice_to_remove)
                ...
                handles.data_selected_for_PRM_resized.z_offset(slice_to_remove) = [];
                handles.data_selected_for_PRM_resized.scan_info(:,4) = handles.data_selected_resized.scan_info(1,4) - numel(slice_to_remove);
            end
        end
    ...
    switch handles.mode
        case 1 % case single mode
            for i = 1:numel(handles.ROI_selected)
                ROIsize = [size(handles.ROI_selected(i).data(1).value) numel(handles.ROI_selected(i).data)];
                factor = handles.resolution_selected/ROIsize(1);
                handles.ROI_selected_resized(i).data.value =zeros([handles.resolution_selected handles.resolution_selected handles.data_selected.scan_info(1,4)]);
                for a=1:numel(handles.ROI_selected(i).data)
                    handles.ROI_selected_resized(i).data.value(:,:,a) = imresize(handles.ROI_selected(i).data(a).value(:,:),factor,'bilinear');
                end
                if isfield(handles, 'ROI_cluster_resized')
                    tmp_cluster =  handles.ROI_cluster_resized;
                    factor_cluster = handles.resolution_selected/size(tmp_cluster.map,1);
                    handles.ROI_cluster_resized.map = zeros([handles.resolution_selected handles.resolution_selected handles.data_selected.scan_info(1,4), 3]);
                    handles.ROI_cluster_resized.trans = zeros([handles.resolution_selected handles.resolution_selected handles.data_selected.scan_info(1,4)]);
                    for a=1:size(handles.ROI_cluster_resized.map, 3)
                        handles.ROI_cluster_resized.trans(:,:,a) = imresize(tmp_cluster.trans(:,:,a),factor_cluster,'nearest');
                        handles.ROI_cluster_resized.map(:,:,a,:) = imresize(tmp_cluster.map(:,:,a,:),factor_cluster,'nearest');
                    end
                end
                % Align ROI slices
                found =0;
                tmp =zeros([handles.resolution_selected handles.resolution_selected handles.data_selected.scan_info(1,4)]);
                for j = 1:numel(handles.ROI_selected(i).data)
                    rank = abs(handles.data_selected.z_offset - handles.ROI_selected(i).scan_z_offset.values(j)) <  1e-3;
                    if ~isempty(find(rank, 1))
                        tmp(:,:,find(rank, 1)) = handles.ROI_selected_resized(i).data.value(:,:,j);
                        found = 1;
                    end
                    
                end
                if found == 1
                    handles.ROI_selected_resized(i).data.value = tmp;
                end
            end
        case 2
            for i = 1:numel(handles.ROI_selected)
                ROIsize = [size(handles.ROI_selected(i).data(1).value) numel(handles.ROI_selected(i).data)];
                factor = handles.resolution_selected/ROIsize(1);
                handles.ROI_selected_resized(i).data.value =zeros([handles.resolution_selected handles.resolution_selected handles.data_selected_for_PRM.scan_info(1,4)]);
                for a=1:numel(handles.ROI_selected(i).data)
                    handles.ROI_selected_resized(i).data.value(:,:,a) = imresize(handles.ROI_selected(i).data(a).value(:,:),factor,'bilinear');
                end
                % Align ROI slices
                tmp =zeros([handles.resolution_selected handles.resolution_selected handles.data_selected_for_PRM.scan_info(1,4)]);
                for j = 1:numel(handles.ROI_selected(i).data)
                    rank = abs(handles.data_selected_for_PRM.z_offset - handles.ROI_selected(i).scan_z_offset.values(j)) < 1e-3;
                    if ~isempty(find(rank, 1))
                        tmp(:,:,find(rank, 1)) = handles.ROI_selected_resized(i).data.value(:,:,j);
                    end
                end
...
% Need to be done to recompute the maps for each clusters
if isfield(handles, 'ROI_cluster_resized')
...
handles = guidata(hObject);
if handles.mode == 1
    MIA_update_axes_single(hObject, eventdata, handles);
else
    MIA_update_axes_PRM(hObject, eventdata, handles)
end


function MIA_update_axes_single(hObject, eventdata, handles)
...
if ~strcmp(get(hObject, 'Tag'), 'MIA_slider_slice') 
    ...
    if (isfield(handles, 'data_loaded') && ~(strcmp(get(hObject, 'Tag'), 'MIA_load_axes') && get(handles.MIA_scan_VOIs_button, 'Value'))) && ...
       ~strcmp(get(hObject, 'Tag'), 'MIA_new_roi')
        handles = MIA_update_image_displayed_single(hObject, eventdata, handles);
    end
    
    % update the ROI matrix (new ROI, resized...)
    if isfield(handles.data_loaded, 'ROI')
        handles = MIA_update_VOI_displayed_single(hObject, eventdata, handles);
    end
    %update MIA_plot1
...
%                     if size(squeeze(handles.data_selected_resized.image(i).reco.data),5) > 3
%                         warndlg('You can not load an RGB image with a MRI scan containing multiple repetition', 'Warning');
%                         handles = MIA_clear_data(hObject, eventdata, handles);
%                         guidata(hObject, handles);
%                         return
%                     end
%                     image(uint8(squeeze(handles.data_selected_resized.image(i).reco.data(:,:,1,slice_nbr,:))),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
...
%                     image(uint8(squeeze(handles.data_displayed.image(:,:,slice_nbr,i,1))),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
%                 else
%                     if( max(~isreal(handles.data_displayed.image(:)))==1 )    % complex data
%                         image(squeeze(abs(handles.data_displayed.image(:,:,slice_nbr,i,1))),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
%                     else    % real data
                        image(squeeze(handles.data_displayed.image(:,:,slice_nbr,i,1)),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
%                     end
%                 end
...
%                     image(uint8(squeeze(handles.data_selected_resized.image(i).reco.data(:,:,1,slice_nbr,:))),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
%                 elseif sum(strcmp(handles.data_selected.list_scan{i}, handles.histo'))>0 && strcmp(handles.data_selected.image(i).reco.color_map, 'Gray')
%                     image(uint8(squeeze(handles.data_displayed.image(:,:,slice_nbr,i,:))),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
%                 else
%                     if( max(~isreal(handles.data_displayed.image(:)))==1 )    % complex data
%                         image(squeeze(abs(handles.data_displayed.image(:,:,slice_nbr,i,1))),'CDataMapping','Scaled','Parent', handles.(sprintf('MIA_data%d', i)),'Tag',sprintf('data%d', i));
%                     else    % real data
...
%                         slice_mask = handles.ROI_selected_resized(x).data.value(:,:,slice_nbr);
%                         ROI_mask = handles.ROI_selected_resized(x).data.value ;
%                         if  sum(strcmp(handles.data_selected.list_scan{i}, handles.histo'))>0 && strcmp(handles.data_selected.image(i).reco.color_map, 'Color')
%                             data_3D =squeeze(handles.data_selected_resized.image(i).reco.data(:,:,get(handles.(sprintf('MIA_data%d_echo_slider', i)), 'Value'),:,get(handles.(sprintf('MIA_data%d_expt_slider', i)), 'Value')));
%                         elseif  sum(strcmp(handles.data_selected.list_scan{i}, handles.histo'))>0 && strcmp(handles.data_selected.image(i).reco.color_map, 'Gray')
%                             data_3D =squeeze(handles.data_selected_resized.image(i).reco.data(:,:,get(handles.(sprintf('MIA_data%d_echo_slider', i)), 'Value'),:,get(handles.(sprintf('MIA_data%d_expt_slider', i)), 'Value')));
%                         else
%                             data_3D=squeeze(handles.data_selected_resized.image(i).reco.data(:,:,get(handles.(sprintf('MIA_data%d_echo_slider', i)), 'Value'),:,get(handles.(sprintf('MIA_data%d_expt_slider', i)), 'Value')));
%                             
%                         end
%                         data_3D(data_3D<=handles.clips{strcmp(handles.clips(:,1),handles.data_selected.list_scan{i}),2}) = NaN;
%                         data_3D(data_3D>=handles.clips{strcmp(handles.clips(:,1),handles.data_selected.list_scan{i}),3}) = NaN;
%                         
%                         mean_VOI = nanmean(data_3D(logical(ROI_mask)));
%                         sd_VOI =  nanstd(data_3D(logical(ROI_mask)));
%                         
%                         % Slice mean / SD
%                         tmp_slice = data_3D(:,:,slice_nbr);
%                         
%                         mean_slice = nanmean(tmp_slice(logical(slice_mask)));
%                         sd_slice =  nanstd(tmp_slice(logical(slice_mask)));
%                         
%                         clear data_slice data_3D
%                         
%                         if i == 1
%                             Slice_values(x*2-1,1) = {[handles.ROI_selected_resized(x).name '-slice-mean']} ;
%                             Slice_values(x*2-1,i+1) = num2cell(mean_slice);
%                             Slice_values(x*2,1) = {[handles.ROI_selected_resized(x).name '-slice-SD']} ;
%                             Slice_values(x*2,i+1) = num2cell(sd_slice);
%                             
%                             VOI_values(x*2-1,1) = {[handles.ROI_selected_resized(x).name '-VOI-mean']} ;
%                             VOI_values(x*2-1,i+1) = num2cell(mean_VOI);
%                             VOI_values(x*2,1) = {[ handles.ROI_selected_resized(x).name '-VOI-SD']} ;
...
function handles = MIA_update_image_displayed_single(hObject, eventdata, handles)
...
function handles = MIA_update_VOI_displayed_single(hObject, eventdata, handles)
% slice_nbr = get(handles.MIA_slider_slice, 'Value');
...
% --------------------------------------------------------------------
function MIA_split_expt_Callback(hObject, eventdata, handles)
% hObject    handle to MIA_split_expt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

if ~isfield(handles, 'database') || get(handles.MIA_scan_VOIs_button,'Value') ==1
    return
end

user_response = questdlg('In which list you want to add your new scans?', 'Bip', 'Time points', 'Scans', 'Cancel', 'Cancel');
if strcmp(user_response, 'Cancel') ==1
    return
end
if strcmp(user_response, 'Time points') ==1
    first_time_point = inputdlg('Name of the firt time point ', 'Question?', 1, {''});
end


id_nbr = get(handles.MIA_name_list, 'Value');
time_point_nbr = get(handles.MIA_time_points_list, 'Value');
scan_nbr = get(handles.MIA_scans_list, 'Value');

fid=fopen(fullfile(handles.database(id_nbr).path, handles.database(id_nbr).day(time_point_nbr).scans_file{scan_nbr}),'r');
if fid>0
    fclose(fid);
    data_loaded = load(fullfile(handles.database(id_nbr).path, handles.database(id_nbr).day(time_point_nbr).scans_file{scan_nbr}));
    if ~or(isfield(data_loaded, 'uvascim'), isfield(data_loaded, 'so2struct'))
        warndlg('Please select a correct uvasc structure');
        clear data_loaded
        return
...
for i=1:data_loaded.uvascim.image.reco.no_expts
    tmp.uvascim = data_loaded.uvascim;
    tmp.uvascim.image.reco.data = data_loaded.uvascim.image.reco.data(:,:,:,:,i);
    tmp.uvascim.image.reco.fov_offsets = data_loaded.uvascim.image.reco.fov_offsets;
    tmp.uvascim.image.reco.fov_orientation = data_loaded.uvascim.image.reco.fov_orientation;
    tmp.uvascim.image.reco.fov_phase_orientation= data_loaded.uvascim.image.reco.fov_phase_orientation;
    tmp.uvascim.image.reco.label= data_loaded.uvascim.image.reco.label;
    tmp.uvascim.image.reco.no_expts = 1;
    tmp.uvascim.image.reco.phaselabel = data_loaded.uvascim.image.reco.phaselabel;
    tmp.uvascim.image.reco.scaling_factor =data_loaded.uvascim.image.reco.scaling_factor;
    tmp.uvascim.image.reco.scaling_offset = data_loaded.uvascim.image.reco.scaling_offset;
    uvascim.image =  tmp.uvascim.image; %#ok<STRNU>
    if strcmp(user_response, 'Time points') ==1
        
        new_file_name = strcat(tmp.uvascim.data_load_dir, handles.database(id_nbr).day(time_point_nbr).parameters{scan_nbr}, '-',...
            num2str(str2double(first_time_point{:})+i-1), '.mat');
    else
        
    end
    %     if exist(new_file_name, 'file') ~= 2
    save(new_file_name,'uvascim');
    %     end
    handles.database(id_nbr).day(time_point_nbr+i).date =num2str(str2double(first_time_point{:})+i-1);
    handles.database(id_nbr).day(time_point_nbr+i).parameters =  [handles.database(id_nbr).day(time_point_nbr+i).parameters handles.database(id_nbr).day(time_point_nbr).parameters(scan_nbr)];
    handles.database(id_nbr).day(time_point_nbr+i).scans_file = [handles.database(id_nbr).day(time_point_nbr+i).scans_file  strcat(handles.database(id_nbr).day(time_point_nbr).parameters(scan_nbr), '-',...
        num2str(str2double(first_time_point{:})+i-1), '.mat')];
   ...
   handles.data_loaded.info_data_loaded.Filename(ROI_data_loaded_idex) = file_name;
      ...
      %% add new ROI data to the database
        new_data = table(categorical(cellstr('Undefined')), categorical(handles.data_loaded.info_data_loaded.Patient(which_image)),...
            ...
            categorical(cellstr('ROI')), false, categorical(newVOI_name),...
...
% if ~isfield(handles.database.Properties.UserData, 'db_pathname')
    MIA_menu_save_database_Callback(hObject, eventdata, handles)
 
% else
    % if a sub-database is displayed (for instance only the group named
    % "control"
%     if isfield(handles, 'database_all')
%         handles.database_all(1).databaseinfo.VOIs = handles.VOIs;
%         handles.database_all(1).databaseinfo.clips = handles.clips;
%         handles.database_all(1).databaseinfo.histo = handles.histo;
%         %     handles.database_all(1).databaseinfo.day = get(handles.patient_graph1_day_button, 'Value');
%         database = handles.database_all;
%     else
%         handles.database(1).databaseinfo.VOIs = handles.VOIs;
%         handles.database(1).databaseinfo.clips = handles.clips;
%         handles.database(1).databaseinfo.histo = handles.histo;
%         database = handles.database;
% %     end
%     
%     choice = questdlg(['Do you want to overwrite '  fullfile(database(1).databaseinfo.pathname, database(1).databaseinfo.filename) '?'],...
%         'Question', 'Yes','No','Abort','Abort');
%     switch choice
%         case 'Yes'
%             save(fullfile(database(1).databaseinfo.pathname, database(1).databaseinfo.filename), 'database');
%         otherwise
%             return
%     end
%     
...
% --------------------------------------------------------------------
function MIA_split_echo_Callback(hObject, eventdata, handles)
% hObject    handle to MIA_split_echo (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
user_response = questdlg(sprintf('Do you want split this parameter in several echoes? \n (The name of echo label will be the new name of parameters)'), 'Split echoes', 'Yes', 'Cancel', 'Cancel');
if strcmp(user_response, 'Cancel') ==1
    return
end
...
if data_loaded.uvascim.image.reco.no_echoes < 2
    warndlg('Not possible: this parameter contain only 1 echo')
    return
end

nb_expts = data_loaded.uvascim.image.reco.no_expts;
for i=1:data_loaded.uvascim.image.reco.no_echoes
    tmp.uvascim = data_loaded.uvascim;
    tmp.uvascim.image.reco.data = data_loaded.uvascim.image.reco.data(:,:,i,:,:);
    tmp.uvascim.image.reco.no_echoes = 1;
    tmp.uvascim.image.reco.echo_label = data_loaded.uvascim.image.reco.echo_label(i);
    tmp.uvascim.image.reco.unit = data_loaded.uvascim.image.reco.unit(i);
    tmp.uvascim.image.reco.fov_offsets = data_loaded.uvascim.image.reco.fov_offsets(:,i,:,1:nb_expts);
    tmp.uvascim.image.reco.fov_orientation = data_loaded.uvascim.image.reco.fov_orientation(:,i,:,1:nb_expts);
    if(size(data_loaded.uvascim.image.reco.fov_phase_orientation,2)>1), tmp.uvascim.image.reco.fov_phase_orientation = data_loaded.uvascim.image.reco.fov_phase_orientation(:,i,1:nb_expts); end
    tmp.uvascim.image.reco.label = data_loaded.uvascim.image.reco.label(:,:,1:nb_expts);
    tmp.uvascim.image.reco.phaselabel = data_loaded.uvascim.image.reco.phaselabel(:,:,1:nb_expts);
    uvascim.image =  tmp.uvascim.image; %#ok<STRNU>
    
    new_file_name = strcat(handles.database(id_nbr).path, handles.database(id_nbr).name,'-', handles.database(id_nbr).day(time_point_nbr).date,'-', tmp.uvascim.image.reco.echo_label, '.mat');
    save(new_file_name{:},'uvascim');
    handles.database(id_nbr).day(time_point_nbr).parameters =  [handles.database(id_nbr).day(time_point_nbr).parameters tmp.uvascim.image.reco.echo_label];
    handles.database(id_nbr).day(time_point_nbr).scans_file = [handles.database(id_nbr).day(time_point_nbr).scans_file strcat(handles.database(id_nbr).name,'-', handles.database(id_nbr).day(time_point_nbr).date,'-', tmp.uvascim.image.reco.echo_label, '.mat')];
end
guidata(hObject, handles);
MIA_update_database_display(hObject, eventdata, handles)
       ...
       if ~isempty(name_selected)
           for m=1:numel(index_data_to_import)
               ...
               if exist(fullfile(MIA_tmp_folder, [NAME, '.json']), 'file')
                   ...
                   new_data = table(categorical(cellstr('Undefined')), categorical(cellstr(name_selected)), categorical(cellstr(tp_selected)), categorical(cellstr([handles.database.Properties.UserData.MIA_data_path, 'Raw_data', filesep])), categorical(cellstr(file_name)),...
...
% --------------------------------------------------------------------
function MIA_menu_pipeline_Callback(hObject, eventdata, handles)
% hObject    handle to MIA_menu_pipeline (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --------------------------------------------------------------------
function MIA_pipeline_Lyon_Grenoble_Callback(hObject, eventdata, handles)
% hObject    handle to MIA_pipeline_Lyon_Grenoble (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% define all the scan available

% selection = questdlg(['Are you sure to run the pipeline Lyon Grenoble on patient: ' handles.data_selected.patient_info.name '-' handles.data_selected.patient_info.timepoint '?'],...
%     'Warning',...
%     'Yes','No','Yes');
...
%
%
% %% Realign and Co-reg the scan acquired in Sagital
% isOpen = findobj('Tag', 'Math_GUI');
% if ~isempty(isOpen)
%     close(findobj('Tag', 'Math_GUI'))
% end
%
% parameters_list = [];
% VOIs_list = [];
...
% parameters_list = unique(parameters_list);
% VOIs_list = unique(VOIs_list);
% MM_objet = Math_module(handles, parameters_list, VOIs_list);
% Data_Math_module =guidata(MM_objet);
%
% scan_names = {'DCE-FA15-SENSE', 'DCE-FA35-SENSE',  'DCE-FA5-SENSE', 'DCE-SENSE', 'DCE-tardif-SENSE'};
% scan_position = [];
%
% for i=1:numel(scan_names)
%     scan_position = [scan_position find(strcmp(parameters_list,scan_names(i)))];
% end
%
% set(Data_Math_module.Math_maps_listbox, 'Value', scan_position)
% set(Data_Math_module.Math_operation_listbox, 'Value', find(strcmp(Data_Math_module.Math_operations_listing', 'SPM: Realign&Coreg')));
% Math_module('Math_operation_listbox_Callback',MM_objet, eventdata, Data_Math_module)
% set(Data_Math_module.Math_additional_info_table, 'Data', {'same' 'SO2_MSME-SENSE' 'Rea-Coreg'});
% Data_Math_module =guidata(MM_objet);
% % open spm widows
% [SPMinter,SPMgraph,~] = spm('FnUIsetup','test',1);
% drawnow
% % run the 'SPM: Realign&Coreg
% Math_module('Math_go_button_Callback',MM_objet, eventdata, Data_Math_module)
% close(findobj('Tag', 'Math_GUI'))
% close(SPMinter)
% close(SPMgraph)
%
% %% Realign the perfusion scan
% handles = guidata(hObject);
% parameters_list = [];
% VOIs_list = [];
% for i=1:numel(get(handles.MIA_name_list, 'String'))
...
% Data_Math_module =guidata(MM_objet);
%
% set(Data_Math_module.Math_maps_listbox, 'Value', find(strcmp(parameters_list,{'PERFUSION-SENSE'})))
% set(Data_Math_module.Math_operation_listbox, 'Value', find(strcmp(Data_Math_module.Math_operations_listing', 'SPM: Realign')));
% Math_module('Math_operation_listbox_Callback',MM_objet, eventdata, Data_Math_module)
% Data_Math_module =guidata(MM_objet);
% % open spm widows
% [SPMinter,SPMgraph,~] = spm('FnUIsetup','test',1);
